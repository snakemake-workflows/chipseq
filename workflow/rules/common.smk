...
def get_trimmed_output(wildcards):
    trimmed_output = []
    for (sample, unit) in units.index:
        if is_single_end(sample, unit):
            trimmed_output.extend(expand(
                    [
...
def get_mapped_filtered_output(wildcards):
    mapped_filtered_output = []
    for sample in samples.index:
        mapped_filtered_output.extend(
            expand (
                [
